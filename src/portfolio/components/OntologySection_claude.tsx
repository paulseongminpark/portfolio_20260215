...
interface Spread { phase: number; title: string; isFirst: boolean; left: string; right: string; rightPhase?: number; rightTitle?: string; }

const PHASE_START = [0, 1, 2, 3, 6, 7, 8, 8];
...
"이제 시작이다"`,
    rightPhase: 8, rightTitle: "지금, 그리고 다음",
  },
];
...
function SpreadPage({ content, phaseLabel, title, mirrorHeader }: {
  content: string;
  phaseLabel?: string;
  title?: string;
  mirrorHeader?: { phaseLabel?: string; title?: string };
}) {
  return (
    <div style={{
      flex: 1,
      ...
      {/* invisible spacer to align with left page header */}
      {mirrorHeader && (
        <div style={{ visibility: "hidden", pointerEvents: "none" }}>
          {mirrorHeader.phaseLabel && (
            <div style={{
              fontFamily: "'Inter', sans-serif",
              fontSize: 11,
              letterSpacing: "0.03em",
              marginBottom: mirrorHeader.title ? 14 : 32,
            }}>
              {mirrorHeader.phaseLabel}
            </div>
          )}
...
function Book({ spreadIdx, onPrev, onNext }: {
  ...
  const phaseLabel = phaseTotal > 1
    ? `Phase ${s.phase} · ${phaseLocal}/${phaseTotal}`
    : `Phase ${s.phase}`;
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        borderRadius: 3,
        boxShadow: "0 2px 16px rgba(0,0,0,0.045), 0 0 0 0.5px rgba(0,0,0,0.04)",
        overflow: "hidden",
        minHeight: 280,
      }}>
        <SpreadPage
          ...
          title={s.isFirst ? s.title : undefined}
        />
        ...
        <SpreadPage
          content={s.right}
          phaseLabel={s.rightPhase ? `Phase ${s.rightPhase}` : undefined}
          title={s.rightTitle}
          mirrorHeader={
            s.rightTitle ? undefined :
            s.isFirst ? { phaseLabel, title: s.title } :
            { phaseLabel }
          }
        />
      </div>
      ...
      <div style={{
        ...
        <button
          ...
          style={{ ...btnBase, opacity: isFirstSpread ? 0.18 : 1, cursor: isFirstSpread ? "default" : "pointer" }}
        >
        ...
        <button
          ...
          style={{ ...btnBase, opacity: isLastSpread ? 0.18 : 1, cursor: isLastSpread ? "default" : "pointer" }}
        >
          다음
          <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
...
export function OntologySection() {
  ...
  const curPhase = SPREADS[spreadIdx].phase - 1;
